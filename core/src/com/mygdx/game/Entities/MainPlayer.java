...
public class MainPlayer extends Player implements Serializable {
    ...
    public MainPlayer(int id, String color, float velocity) {
        ...
        camera = FasaarGame.scene.cam;
        camera.position.set(serverPlayer.getPosition());
        setPosition(camera.position);
        setPosition(serverPlayer.getPosition());
        controller = new FirstPersonCameraController(FasaarGame.scene.cam);
    ...
    public void update() {
        ...
        if ((position.x > waterPosition.x && position.x < waterPosition.x + waterSize) &&
            ...
            FasaarGame.fontBatch.setColor(1, 1, 1, 0.25f);
            FasaarGame.fontBatch.draw(new Texture("water_overlay.png"), 0, 0, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
        }
        ...
        FasaarGame.client.sendTCP(this.serverPlayer);
    }
